"""
Scrcpy控制器模块。
提供通过Scrcpy控制Android设备的功能。
Scrcpy是一个强大的Android设备显示和控制工具，通过ADB提供设备控制功能。
"""
import time
...
class ScrcpyController(DeviceController):
    """
    Scrcpy控制器。
    提供通过Scrcpy控制Android设备的功能。
    """
    ...
    def scrcpy_init(self) -> None:
        ...
        self.device.shell('scrcpy --no-display --no-audio --no-control')
        
        # 建立连接
        self.scrcpy = self.device.shell('scrcpy', stream=True)
    ...
    def click(self, x: int, y: int) -> None:
        ...
        """
        # 发送点击命令
        self.scrcpy.write(f'input tap {x} {y}\n'.encode())
        self.scrcpy.flush()
    ...
    def long_click(self, x: int, y: int, duration: float = 1.0) -> None:
        ...
        """
        # 发送长按命令
        self.scrcpy.write(f'input swipe {x} {y} {x} {y} {int(duration * 1000)}\n'.encode())
        self.scrcpy.flush()
    ...
    def swipe(self, x1: int, y1: int, x2: int, y2: int, duration: float = 0.1) -> None:
        ...
        """
        # 发送滑动命令
        self.scrcpy.write(f'input swipe {x1} {y1} {x2} {y2} {int(duration * 1000)}\n'.encode())
        self.scrcpy.flush()
