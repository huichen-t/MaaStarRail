...
MaaTouch是一个类似minitouch的触摸控制工具，但提供了更多的功能和更好的性能。
"""
import time
...
class MaaTouchController(DeviceController):
    """
    MaaTouch控制器。
    提供通过MaaTouch控制Android设备的功能。
    """
    ...
    def maatouch_init(self) -> None:
        ...
        # 启动MaaTouch服务
        self.device.shell('pkill -f maatouch')
        self.device.shell('maatouch')
        
        # 建立连接
        self.maatouch = self.device.shell('maatouch', stream=True)
        
        # 等待服务启动
        time.sleep(0.5)
        
        # 发送初始化命令
        self.maatouch.write(b'v 1\n')
        self.maatouch.write(b'^ 1\n')
        self.maatouch.write(b'$ 1\n')
        self.maatouch.flush()
    ...
    def click(self, x: int, y: int) -> None:
        ...
        """
        # 发送按下命令
        self.maatouch.write(f'd 0 {x} {y} 50\n'.encode())
        self.maatouch.flush()
        time.sleep(0.05)
        
        # 发送抬起命令
        self.maatouch.write(b'u 0\n')
        self.maatouch.flush()
    ...
    def swipe(self, x1: int, y1: int, x2: int, y2: int, duration: float = 0.1) -> None:
        ...
        # 发送按下命令
        self.maatouch.write(f'd 0 {x1} {y1} 50\n'.encode())
        self.maatouch.flush()
        ...
        for i in range(1, steps + 1):
            x = int(x1 + dx * i)
            y = int(y1 + dy * i)
            self.maatouch.write(f'm 0 {x} {y} 50\n'.encode())
            self.maatouch.flush()
